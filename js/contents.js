...
try{
	...
	error:function (XMLHttpRequest,status,e){
		swal("获取课表错误！", "呜呜……小阿狸发现教务网好像出了什么问题~主人快检查下能否正常进入教务网哦~", "error");
	}
	}).responseText; 
	NProgress.done();
...
//console.log (busytime);
	...
	swal("上课时间获取模块错误！", "呜呜……/(ㄒoㄒ)/~小阿狸崩溃啦，主人检查下浏览器版本是否最新哦，或者联系作者详细描述错误情形以帮助改进插件吧~\n\n错误信息：\n"+err, "error");
	console.log(err);
}
...
if (Showscore=="enabled"){
	//chrome.extension.sendMessage({cmd: "update",progress:progress},function(response) {});
		...
		if (!title) {    
			...
			title.innerHTML = "萌萌哒小阿狸正在为你努力地工作哦……o(〃'▽'〃)o已完成："+Math.round(progress)+"%";
			document.getElementsByTagName('head')[0].appendChild(title);
		}
		else {title.innerHTML = "萌萌哒小阿狸正在为你努力地工作哦……o(〃'▽'〃)o已完成："+Math.round(progress)+"%";}
	
	if (progress==100){
		title.innerHTML = "小阿狸已经完成主人交给的任务了呢~\(^o^)/！现在主人可以很方便地选课啦~";
	...
	chrome.extension.sendMessage({cmd: "finish"},function(response) {});
		...
		else {title.innerHTML = "小阿狸已经完成主人交给的任务了呢~\(^o^)/！现在主人可以很方便地选课啦~";}
		NProgress.done(); 
}
	//chrome.extension.sendMessage({cmd: "update",progress:progress},function(response) {});
...
{
	swal("时间冲突判断模块错误！", "呜呜……/(ㄒoㄒ)/~小阿狸崩溃啦，主人检查下浏览器版本是否最新哦，或者联系作者详细描述错误情形以帮助改进插件吧~\n\n错误信息：\n"+err, "error");
	console.log(err);
}
}
...
{
	swal("课程余量判断模块错误！", "呜呜……/(ㄒoㄒ)/~小阿狸崩溃啦，主人检查下浏览器版本是否最新哦，或者联系作者详细描述错误情形以帮助改进插件吧~", "error");
}
}
...
$.ajax({ 
	type:"GET", 
	dataType: "json", 
	url: "http://115.159.96.65/alienrollment/search.php?name="+teacher,
		error:function(){   
...
{
	console.log(err);
	swal("教师评分及均绩查询模块错误！", "呜呜……/(ㄒoㄒ)/~小阿狸崩溃啦，主人检查下浏览器版本是否最新哦，或者联系作者详细描述错误情形以帮助改进插件吧~", "error");
}
}
...
{
	swal("Main函数错误！", "呜呜……/(ㄒoㄒ)/~小阿狸崩溃啦，主人检查下浏览器版本是否最新哦，或者联系作者详细描述错误情形以帮助改进插件吧~\n\n错误信息：\n"+err, "error");
	console.log(err);
}
}
...
{
	swal("主程序错误！", "呜呜……/(ㄒoㄒ)/~小阿狸崩溃啦，主人检查下浏览器版本是否最新哦，或者联系作者详细描述错误情形以帮助改进插件吧~\n\n错误信息：\n"+err, "error");
	console.log(err);
}
}
});
