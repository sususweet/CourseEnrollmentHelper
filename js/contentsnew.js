...
function doScrollCheck2() {
    ...
    if (courseitem.length==0) {
        return setTimeout(doScrollCheck2, 50);
    }else{
		Addbtn();
...
function Superlookup(id,btnstate) {
	...
	$.ajax({
	　　url: "http://zjuchoose.usa3v.com/permit/stat.asp?uid="+student+"&type=0",
	　　type : 'GET'
	});
...
$(document).ready(function() { 
	...
	if (NewVer=="enabled"){
		if(permit==1){
			...
			NewVer="disabled";
			swal("插件功能受限！", '新系统适配还在内测阶段哦~\n酷爱来这里申请内测权限吧~:http://course.zjuqsc.com/alienrollment/apply.php', "warning");
